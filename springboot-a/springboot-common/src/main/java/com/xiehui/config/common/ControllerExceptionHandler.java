...
import com.xiehui.common.core.response.KnowledgeResponse;
...
public class ControllerExceptionHandler {
    ...
    private static final SerializeConfig JSON_SERIALIZE_CONFIG = new SerializeConfig();
    static {
        JSON_SERIALIZE_CONFIG.config(Long.class, SerializerFeature.WriteNonStringValueAsString, true);
    }
    ...
    public KnowledgeResponse handleParameterValidException(Exception exception, HttpServletRequest request) {
        // 打印日志提示
    ...
    public KnowledgeResponse<Object> handleUnexpectedServerError(Throwable throwable, HttpServletRequest request) {
        // 打印日志提示
        if (Objects.nonNull(request)) {
        ...
        if (throwable instanceof KnowledgeException) {
            if (RunMode.ONLINE.equals(mode)) {
        ...
        // 处理异常信息
        KnowledgeResponse<Object> response = new KnowledgeResponse<>();
        if (throwable instanceof KnowledgeException) {
        	KnowledgeException exception = (KnowledgeException)throwable;
            response.setCode(exception.getCode().getValue());
            response.setMessage(exception.getMessage());
            if (Objects.nonNull(exception.getReturnData())) {
                response.setData(JSON.toJSON(exception.getReturnData(), JSON_SERIALIZE_CONFIG));
            }
        } else {
            response.setCode(ExceptionCode.COMMON_CODE.getValue());
